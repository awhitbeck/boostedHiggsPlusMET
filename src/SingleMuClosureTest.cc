...
int main(int argc, char** argv){
  ...
  TFile* efficFile = new TFile("Efficiencies.root","read");
  TH1F* isoEff = (TH1F*) efficFile->Get("Efficiencies/MuIsoActivityPT/MuIsoActivityPT");
  TH1F* recoEff = (TH1F*) efficFile->Get("Efficiencies/MuRecoActivityPT/MuRecoActivityPT");
  TH1F* accEff = (TH1F*) efficFile->Get("Efficiencies/MuAccHTMHT/MuAccHTMHT");
  // need filler functions for lepton isolation and activity, and MHT
  ...
  for( int iSample = 0 ; iSample < skimsMu.ntuples.size() ; iSample++){
    ...
    for( int iEvt = 0 ; iEvt < numEvents ; iEvt++ ){
      ...
      if(!baselineCut(ntuple)) continue; 
      if( taggingCut(ntuple) ){
	double MET = fillMET(ntuple);
	double MHT = fillMHT(ntuple);
	double HT = fillHT(ntuple);
	double accEffVal = accEff->GetBinContent(accEff->FindBin(HT,MHT));
	if( accEffVal == 0. )
	  accEffVal = 1.;
	double lepPt = fillLepPt(ntuple);
	double lepActivity = fillLepActivity(ntuple);
	if( lepActivity < 0. ) lepActivity = 0. ;
	double recoEffVal = recoEff->GetBinContent(recoEff->FindBin(lepActivity,lepPt));
	double isoEffVal = isoEff->GetBinContent(isoEff->FindBin(lepActivity,lepPt));
	double totalWeight = (1.-isoEffVal)/isoEffVal+(1.-recoEffVal)/recoEffVal/isoEffVal+(1.-accEffVal)/accEffVal/recoEffVal/isoEffVal;
	double testMass = arbitrationFunc(ntuple);
	if( totalWeight > 2. ) {
	  cout << "- - - - - - - - - - - - - - - - - - - - - - " << endl;
	  cout << "New Event: " << endl;
	  cout << "MET: " << MET << endl;
	  cout << "MHT: " << MHT << " HT: " << HT << endl;
	  cout << "accEff: " << accEffVal << endl;
	  cout << "lep pt: " << lepPt << " lep act.: " << lepActivity << endl;
	  cout << "recoEffVal: " << recoEffVal << " isoEffVal: " << isoEffVal << endl;
	...
	mu_versusMJ.fill(ntuple,totalWeight);
      }
    }
  }
  ...
  */
  ...
  can->SaveAs("SingleMuclosureTest_J1ptArbitration/closureTest.png");
