...
public: 
  ...
  void Write(ntupleType* ntuple){
    if( histo2dMap.find(ntuple) != histo2dMap.end() )
      histo2dMap[ntuple]->Write();

    if( signalHisto2dMap.find(ntuple) != signalHisto2dMap.end() )
      signalHisto2dMap[ntuple]->Write();
