...
int main(int argc, char** argv){
    ...
    plot METplot(*fillMET<RA2bTree>,"MET_singleMuCR_singleHiggsTag","MET [GeV]",16,100.,1600.);
    plot nMuonplot(*fillNumMuons<RA2bTree>,"NumMuons_singleMuCR_singleHiggsTag","n_{#mu}",5,-0.5,4.5);
    plot MTplot(*computeMuonMT<RA2bTree>,"MT_singleMuCR_singleHiggsTag","M_{T} [GeV]",15,0.,300.);
    ...
    for( int iPlot = 0 ; iPlot < plots.size() ; iPlot++){
        plots[iPlot].addDataNtuple(ntuple,"data_SLm");
    }
    ...
    for( int iEvt = 0 ; iEvt < numEvents ; iEvt++ ){
        ntuple->GetEntry(iEvt);
        if( iEvt % 1000000 == 0 ) cout << "data_singleMu: " << iEvt << "/" << numEvents << endl;
        if(! singleMuBaselineCut(ntuple) ) continue;
        if(! antiTaggingLooseCut(ntuple) ) continue;    
        if( ntuple->TriggerPass->size() < 25 || ( !ntuple->TriggerPass->at(21) && !ntuple->TriggerPass->at(22) && !ntuple->TriggerPass->at(23) && !ntuple->TriggerPass->at(24)) ) continue;
        for( int iPlot = 0 ; iPlot < plots.size() ; iPlot++){
    ...
    for( int iPlot = 0 ; iPlot < plots.size() ; iPlot++){
        ...
        plots[iPlot].Draw(can,skims.ntuples,skims.signalNtuples,"../plots/plotObs_singleMuCR_antitag_plots",0.1,2.0,true);
    }
}
