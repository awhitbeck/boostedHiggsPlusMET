...
void SetRatioErr(TH1F*TotalBkg, TH1F*Pred, TGraphAsymmErrors &Closure){
    ...
    TH2F* kappaDist = new TH2F("kappaDist","kappaDist",3,300.,900.,1000,0.,4.);
    kappaDist->Reset();
    for( int m=1; m<=3; ++m){
...
void UnblindingControl(TString tag = "", bool doubleHiggsRegion = true, TString baseDir="./",bool applySF=true){
	...
	for( int b = 2; b <= numMETbins+2 ; b++ ){
		...
		float QCDErr=sqrt((QCD->GetBinError(b)/QCD->GetBinContent(b))*(QCD->GetBinError(b)/QCD->GetBinContent(b))+(LDPSFErr_2H[r]/LDPSF_2H[r]*LDPSFErr_2H[r]/LDPSF_2H[r]))*LDPSF_2H[r]*QCD->GetBinContent(b);
		if(MCQCD->GetBinContent(b)<0.000001)QCDErr=LDPSFErr_2H[r];
		float ZErr=sqrt((ZJets->GetBinError(b)/ZJets->GetBinContent(b))*(ZJets->GetBinError(b)/ZJets->GetBinContent(b))+(PhoSFErr_2H[r]/PhoSF_2H[r]*PhoSFErr_2H[r]/PhoSF_2H[r]))*PhoSF_2H[r]*ZJets->GetBinContent(b);
		float WErr=sqrt((WJets->GetBinError(b)/WJets->GetBinContent(b))*(WJets->GetBinError(b)/WJets->GetBinContent(b))+(SLSFErr_2H[r]/SLSF_2H[r]*SLSFErr_2H[r]/SLSF_2H[r]))*SLSF_2H[r]*WJets->GetBinContent(b);
		float TErr=sqrt((TTJets->GetBinError(b)/TTJets->GetBinContent(b))*(TTJets->GetBinError(b)/TTJets->GetBinContent(b))+(SLSFErr_2H[r]/SLSF_2H[r]*SLSFErr_2H[r]/SLSF_2H[r]))*SLSF_2H[r]*TTJets->GetBinContent(b);

		MCQCD->SetBinError(b-1,QCDErr);
		MCZJets->SetBinError(b-1,ZErr);
		MCWJets->SetBinError(b-1,WErr);
		MCTTJets->SetBinError(b-1,TErr);
	}
	if( ! doubleHiggsRegion ){
   ...
   TGraphAsymmErrors* grClose=new TGraphAsymmErrors(Closure); 
    ...
    botPad->Draw();
    topPad->cd();
    MCprediction->GetYaxis()->SetRangeUser(0.0,4.0);
   ...
   //MCprediction->Draw("e2same");
    ...
    leg->AddEntry(Dataprediction,"Data Prediction ","p");
    leg->AddEntry(MCprediction,"MC Prediction ","f");
    leg->Draw();
 ...
 KappaNom_1->SetPoint(1,9999999,1);
    ...
    if( doubleHiggsRegion ){
        can->SaveAs("Unblinding_doubletagSRRegionTotal.png");
        can->SaveAs("Unblinding_doubletagSRRegionTotal.pdf");
        can->SaveAs("Unblinding_doubletagSRRegionTotal.eps");        
    }else{
        can->SaveAs("Unblinding_singletagSRRegionTotal.png");
        can->SaveAs("Unblinding_singletagSRRegionTotal.pdf");
        can->SaveAs("Unblinding_singletagSRRegionTotal.eps");        
    }
}
