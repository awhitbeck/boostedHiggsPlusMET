...
void computeIntegratedRatio(TH1F* num, TH1F* denom, int start_bin=1, int end_bin=4,bool verbose=false){
    ...
    SetRatioErr(num_int,denom_int,graph,1,100.,1000.,verbose);
...
void computeScaleFactor(TH1F* hdata,TH1F* hmc, bool verbose=false){
    ...
    yieldmc = hmc->IntegralAndError(1,4,errmc) ; 
    yielddata = hdata->IntegralAndError(1,4,errdata) ;
    if( verbose ){
        if( yielddata == 0. )
...
void checkScaleFactors(TString tag = "_singleMu", bool doubletag = true, TString baseDir="./",int integrate_from=1,int integrate_to=4){
    ...
    if( doubletag )
        ...
        cout << "tagSR:" << endl;
    SetRatioErr(doubletagSRdata,doubletagSRmc,MCRatio,4,100.,900.,true);
    setStyle(MCRatio);
    if( tag == "_lowDphi" || tag == "_looseCuts_lowDphi" )
    ...
    if( doubletag )
        ...
        cout << "tagSB: " << endl;
    SetRatioErr(doubletagSBdata,doubletagSBmc,MCRatio,4,100.,900.,true);
    setStyle(MCRatio);
    if( tag == "_lowDphi" || tag == "_looseCuts_lowDphi" )
        MCRatio.GetXaxis()->SetLimits(300.,900.);
    MCRatio.Draw("A,p");
    ...
    cout << "antitagSR:" << endl;
    SetRatioErr(antitagSRdata,antitagSRmc,MCRatio,4,100.,900.,true);
    setStyle(MCRatio);
    if( tag == "_lowDphi" || tag == "_looseCuts_lowDphi" )
        MCRatio.GetXaxis()->SetLimits(300.,900.);
    MCRatio.Draw("A,p");
    ...
    cout << "antitagSB: " << endl;
    SetRatioErr(antitagSBdata,antitagSBmc,MCRatio,4,100.,900.,true);
    setStyle(MCRatio);
    if( tag == "_lowDphi" || tag == "_looseCuts_lowDphi" )
        MCRatio.GetXaxis()->SetLimits(300.,900.);
    MCRatio.Draw("A,p");
