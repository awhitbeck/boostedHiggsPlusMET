...
int main(int argc, char** argv){
  ...
  TFile* outputFile = new TFile("NJets4_BTags1_METonlyBinning/datacardInputs.root","RECREATE");
  TCanvas* can = new TCanvas("can","can",500,500);
  for( int iPlot = 0 ; iPlot < plots.size() ; iPlot++){
    plots[iPlot].buildSum();
    plots[iPlot].Draw(can,skims.ntuples,skims.signalNtuples,"NJets4_BTags1_METonlyBinning");
    outputFile->cd();
    plots[iPlot].Write();
  }
  outputFile->Close();
