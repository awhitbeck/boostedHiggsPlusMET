from ROOT import *
finDC=TFile("datacardInputsUnblinding.root","READ");
finDCSFup=TFile("datacardInputsUnblindingDoublebSFUp.root","READ");
finDCSFdn=TFile("datacardInputsUnblindingDoublebSFDn.root","READ");
finDCSmear=TFile("datacardInputsUnblinding.root","READ");
finDCSmearUp=TFile("datacardInputsUnblindingResSFUp.root","READ");
#LOOK AT DOUBLE TAG FOR Maximal effect
finNom=TFile("datacardInputsUnblinding.root","READ")
...
for m in ModelPoints:
	for r in Regions:
		NominalSF=finNom.Get("AnalysisMETT5HH_%s_T5HH%d" %(r,m))
		SFUp=finDCSFup.Get("AnalysisMETT5HH_%s_T5HH%d" %(r,m))
		SFDn=finDCSFdn.Get("AnalysisMETT5HH_%s_T5HH%d" %(r,m))
		MassResSmear=finDCSmear.Get("AnalysisMETT5HH_%s_T5HH%d" %(r,m))
		MassResSmearUp=finDCSmearUp.Get("AnalysisMETT5HH_%s_T5HH%d" %(r,m))
		BBTagSF=NominalSF.Clone("BBTagSF_%s_%d" %(r,m))
		BBTagSFDn=NominalSF.Clone("BBTagSFDn_%s_%d" %(r,m))
		...
		for i in range(3):
			...
			BBTagSF.SetBinContent(i+1, Unc)
		        if NominalSF.GetBinContent(i+1)<0.000001:Unc=1.0
                        else: Unc=SFDn.GetBinContent(i+1)/NominalSF.GetBinContent(i+1)	
			BBTagSFDn.SetBinContent(i+1, Unc);
			if(NominalSF.GetBinContent(i+1)>0):
				MCStatErr.SetBinContent(i+1,1.0+ NominalSF.GetBinError(i+1)/NominalSF.GetBinContent(i+1))
				MassResErr.SetBinContent(i+1,MassResSmearUp.GetBinContent(i+1)/NominalSF.GetBinContent(i+1))
				
			else: 
				MCStatErr.SetBinContent(i+1, 1.0);
				MassResErr.SetBinContent(i+1,1.0);
		...
		BBTagSFDn.Write("BBTagSFDn_%s_T5HH%d" %(r,m))
...
for m in ModelPoints:
	for r in Regions:
		...
		SFDn=finDCSFdn.Get("AnalysisMETT5HZ_%s_T5HH%d" %(r,m))
		...
		for i in range(3):
			...
			#print m,r,i, MassResSmearUp.GetBinContent(i+1)/NominalSF.GetBinContent(i+1) 
			BBTagSF.SetBinContent(i+1, Unc)
		        if NominalSF.GetBinContent(i+1)<0.000001:Unc=1.0
                        else: Unc=SFDn.GetBinContent(i+1)/NominalSF.GetBinContent(i+1)	
			BBTagSFDn.SetBinContent(i+1, Unc);
		...
		BBTagSFDn.Write("BBTagSFDn_%s_T5HZ%d" %(r,m))
...
for m in ModelPoints:
	...
	JERUp=T5HH_JERUp.Clone("JERUp_%d" %m)
	JERDn=T5HH_JERDn.Clone("JERDn_%d" %m)
	JECUp=T5HH_JECUp.Clone("JECUp_%d" %m)
	JECDn=T5HH_JECDn.Clone("JECDn_%d" %m)
	ScaleUp=T5HH_ScaleUp.Clone("ScaleUp_%d" %m)
	ScaleDn=T5HH_ScaleDn.Clone("ScaleDn_%d" %m)
	ISRDn=T5HH_ISRDn.Clone("ISRDn_%d" %m)
	ISRUp=T5HH_ISRUp.Clone("ISRUp_%d" %m)
	for i in range(3):
